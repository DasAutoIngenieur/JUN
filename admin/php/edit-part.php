...
<body>
        ...
        <div class="flex-items-center justify-center text-left py-6 px-28">
            <div class="max-w-md mx-auto mt-10 bg-white p-6 py-5 rounded-lg shadow-md">
                ...
                <form action="#" method="POST">
                    ...
                    <div class="mb-6">
                        <label for="name" class="exo-2-regular text-gray-700 font-medium mb-2">Part</label>
                        <input type="text" id="name" name="part" class="exo-2-regular w-full px-4 py-2 rounded-lg shadow-sm focus:outline-none">
                    </div>
    ...
    <?php
        ...
        $id = $_GET['id'];
        $val = $con->query("Select * from parts where id=$id");
        $data = mysqli_fetch_array($val);
            ...
            if(isset($_POST['sub'])) {
                ...
                $part_part=$_POST['part'];
                
                $con->query("update parts set name='$part_name'
    ,email='$part_email',phone='$part_phone',part='$part_part' where id=$id");

...
<body>
            ...
            <div class="max-w-md mx-auto mt-10 bg-white p-6 py-5 rounded-lg shadow-md">
                ...
                <form action="#" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="name" name="request" class="exo-2-regular w-full px-4 py-2 rounded-lg shadow-sm focus:outline-none">
                    </div>
    ...
    <?php
        ...
        $id = $_GET['id'];
        $val = $con->query("Select * from demo where id=$id");
        $data = mysqli_fetch_array($val);

            if(isset($_POST['sub'])) {
                $demo_name=$_POST['name'];
                $demo_email=$_POST['email'];
                $demo_req=$_POST['request'];
                
                $con->query("update parts set name='$demo_name'
    ,email='$demo_email',request='$demo_req' where id=$id");
